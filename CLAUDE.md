...
**Template source (`TEMPLATE_SOURCE`):** by default the MA/AR `.docx` are read from the bundled `_BUNDLED_{AR,MA}_DIR` next to the exe. `_resolve_template_dirs()` (called once right after `st.set_page_config`, which sets the `AR_TEMPLATE_DIR`/`MA_TEMPLATE_DIR` globals `resolve_template` reads) supports two alternatives: `sharepoint` downloads the latest templates from the EY SharePoint library at startup, and `onedrive` reads from a locally synced/mapped copy at `TEMPLATE_BASE_PATH`. **SharePoint mode uses no stored credential / app registration** — `_sp_session()` reuses the user's existing browser sign-in via `browser_cookie3` (optional dep), `_sync_sharepoint_templates()` (memoised with `@st.cache_resource`; sidebar "Refresh templates" button calls `.clear()`) lists + downloads the `.docx` via the SharePoint REST API into a temp cache, rejecting any non-`PK`/`.docx` payload (an auth-redirect HTML page). Any failure — auth, network, missing folder — falls back to bundled templates with a UI warning (`TEMPLATE_DIR_WARNING`). `template_index.xlsx` and `LETTERHEAD_DIR` always stay bundled. Per-machine settings (`TEMPLATE_SOURCE`, `TEMPLATE_BASE_PATH`, `SHAREPOINT_*`) come from a runtime `.env` next to the exe — `launcher-win.py` loads it with `override=False` so the baked API keys win — and are **not** compiled into `_bundled_config.py` (they're not secrets). The SharePoint path is untested against the live tenant; verify on a real EY machine.
